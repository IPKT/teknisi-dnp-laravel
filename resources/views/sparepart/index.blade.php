@extends('layouts.app',['title' => ''])

{{-- @section('title', 'Data Peralatan') --}}

@section('content')
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4>Data Sparepart</h4>
    <a href="#" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#modalTambahSparepart">Tambah
                    </a>
</div>

@if (session('success'))
<div class="alert alert-success alert-dismissible fade show" role="alert">
    {{ session('success') }}
    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
</div>
@endif

<div class="card shadow-sm">
    <div class="card-body">
        <div class="table-responsive">
            <table id="sparepart-table" class="table table-bordered table-hover">
                <thead class="table-dark">
                    <tr>
                        <th>#</th>
                        <th>Jenis Barang</th>
                        <th>Merk</th>
                        <th>Tipe</th>
                        <th>SN</th>
                        <th>Jenis Peralatan</th>
                        <th>Sumber</th>
                        <th>Status</th>
                        <th></th>
                    </tr>
                </thead>
                   <tbody id="bodySparepartTable">
                            @forelse($spareparts as $s)
                                <tr>
                                    <td>{{$loop->iteration}}</td>
                                    <td>{{$s->jenis_barang}}</td>
                                    <td>{{ $s->merk }}</td>
                                    <td>{{ $s->tipe }}</td> 
                                    <td>{{ $s->serial_number }}</td> 
                                    <td>{{ $s->jenis_peralatan }}</td> 
                                    <td>{{$s->sumber_pengadaan}}</td>
                                    <td>{{$s->status}}</td>
                                    <td>
                                        <div class="dropdown">
                                            <button class="btn btn-sm btn-light" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                                            <i class="bi bi-three-dots-vertical"></i>
                                            </button>
                                            <ul class="dropdown-menu">
                                            <li class=""><a href="#" class="dropdown-item btn-edit-sparepart small py-1" data-id="{{$s->id}}">Edit</a></li>
                                            <li><a href="#" class="dropdown-item btn-delete-sparepart small py-1" data-id="{{$s->id}}">Hapus</a></li>
                                            </ul>
                                        </div>
                                    </td>     
                                </tr>
                            @empty
                                {{-- <tr><td colspan="6" class="text-center text-muted">Belum ada dokumen penting</td></tr> --}}
                                 <tr>
                                    <td>1</td>
                                    <td>-</td>
                                    <td>-</td>
                                    <td>-</td>
                                    <td>-</td>
                                    <td>-</td>
                                </tr>
                            @endforelse
                        </tbody>
            </table>
        </div> 
    </div>
</div>

@include('sparepart.modal_tambah')
@include('sparepart.modal_edit')

@endsection

@section('scripts')
{{-- <script>
    document.getElementById('formTambahSparepart').addEventListener('submit', function(e) {
        e.preventDefault();

        const form = e.target;
        const formData = new FormData(form);
        console.log(formData);

        fetch("{{ route('sparepart.store') }}", {
            method: "POST",
            headers: {
                'X-CSRF-TOKEN': document.querySelector('input[name="_token"]').value
            },
            body: formData
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                // location.reload();
                // Tambahkan baris baru ke tabel
                const tbody = document.getElementById('bodySparepartTable');
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td>${tbody.children.length + 1}</td>
                    <td>${data.sparepart.jenis_barang}</td>
                    <td>${data.sparepart.merk}</td>
                    <td>${data.sparepart.tipe}</td>
                    <td>${data.sparepart.serial_number}</td>
                    <td>${data.sparepart.jenis_peralatan}</td> 
                    <td>Aksi</td>     
                `;
                tbody.appendChild(row);

                // Reset form dan tutup modal
                form.reset();
                const modal = bootstrap.Modal.getInstance(document.getElementById('modalTambahSparepart'));
                modal.hide();
                // tambahEventListenerBtnEdit();
                // tambahEventListenerBtnDelete();
            } else {
                alert('Gagal menyimpan data');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            alert('Terjadi kesalahan saat menyimpan');
        });
    });

</script> --}}

{{-- Tambah Sparepart --}}
<script>
document.getElementById('selectStatusSparepart').addEventListener('change', function(e) {
// const pelepasanInput = document.getElementById('inputTanggalPelepasan');
const inputTanggalKeluar = document.getElementById('inputTanggalKeluar');
const divTanggalKeluar = document.getElementById('divTanggalKeluar');
const inputLokasiPengiriman = document.getElementById('inputLokasiPengiriman');
const divLokasiPengiriman = document.getElementById('divLokasiPengiriman');
const inputLokasiPemasangan = document.getElementById('inputLokasiPemasangan');
const divLokasiPemasangan = document.getElementById('divLokasiPemasangan');
if (e.target.value === 'ready') {
    divTanggalKeluar.style.display = 'none';
    inputTanggalKeluar.required = false;
    divLokasiPengiriman.style.display = 'none';
    inputLokasiPengiriman.required = false;
    divLokasiPemasangan.style.display = 'none';
    inputLokasiPemasangan.required = false;
}else if(e.target.value === 'terpasang'){
    divTanggalKeluar.style.display = 'block';
    inputTanggalKeluar.required = true;
    divLokasiPemasangan.style.display = 'block';
    inputLokasiPemasangan.required = true;
    divLokasiPengiriman.style.display = 'none';
    inputLokasiPengiriman.required = false;
    // pelepasanInput.value = ''; // optional: reset value
}
else if(e.target.value === 'terkirim'){
    divTanggalKeluar.style.display = 'block';
    inputTanggalKeluar.required = true;
    divLokasiPemasangan.style.display = 'none';
    inputLokasiPemasangan.required = false;
    divLokasiPengiriman.style.display = 'block';
    inputLokasiPengiriman.required = true;
    // pelepasanInput.value = ''; // optional: reset value
}
});

document.getElementById('formTambahSparepart').addEventListener('submit', function(e) {
    e.preventDefault();

    const form = e.target;
    const formData = new FormData(form);

    fetch("{{ route('sparepart.store') }}", {
        method: "POST",
        headers: {
            'X-CSRF-TOKEN': document.querySelector('input[name="_token"]').value
        },
        body: formData
    })
    .then(async response => {
        const status = response.status;
        const text = await response.text();

        if (!response.ok) {
            console.error(`HTTP ${status} Error:\n`, text);
            alert(`Gagal menyimpan data (HTTP ${status}):\n` + text);
            throw new Error(`HTTP ${status}`);
        }

        try {
            const data = JSON.parse(text);
            if (data.success) {
                const tbody = document.getElementById('bodySparepartTable');
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td>${tbody.children.length + 1}</td>
                    <td>${data.sparepart.jenis_barang}</td>
                    <td>${data.sparepart.merk}</td>
                    <td>${data.sparepart.tipe}</td>
                    <td>${data.sparepart.serial_number}</td>
                    <td>${data.sparepart.jenis_peralatan}</td>
                    <td>${data.sparepart.sumber_pengadaan}</td>
                    <td>${data.sparepart.status}</td>
                    <td>
                        <div class="dropdown">
                            <button class="btn btn-sm btn-light" type="button" data-bs-toggle="dropdown" aria-expanded="false">
                            <i class="bi bi-three-dots-vertical"></i>
                            </button>
                            <ul class="dropdown-menu">
                            <li><a href="#" class="dropdown-item btn-edit" data-id="">Edit</a></li>
                            <li><a href="#" class="dropdown-item btn-delete" data-id="">Hapus</a></li>
                            </ul>
                        </div>
                    </td>
                `;
                tbody.appendChild(row);

                form.reset();
                const modal = bootstrap.Modal.getInstance(document.getElementById('modalTambahSparepart'));
                modal.hide();
            } else {
                console.warn('Respon sukses tapi flag success = false:', data);
                alert('Gagal menyimpan data: ' + JSON.stringify(data));
            }
        } catch (err) {
            console.error('Gagal parsing JSON:', err, '\nRaw response:', text);
            alert('Respon tidak bisa diproses:\n' + text);
        }
    })
    .catch(error => {
        console.error('Fetch error:', error);
        alert('Terjadi kesalahan saat menyimpan:\n' + error.message);
    });
});
</script>
{{-- END Tambah Sparepart --}}

{{-- Edit Sparepart --}}
<script>
function tambahEventListenerBtnEdit(){
document.querySelectorAll('.btn-edit-sparepart').forEach(btn => {
    btn.addEventListener('click', function () {
        const id = this.dataset.id;
        fetch(`/sparepart/${id}`)
            .then(res => res.json())
            .then(data => {
                document.getElementById('edit-sparepart-id').value = data.id;
                document.getElementById('edit-sparepart-jenis-barang').value = data.jenis_barang;
                document.getElementById('edit-sparepart-jenis-peralatan').value = data.jenis_peralatan;
                document.getElementById('edit-sparepart-tahun-masuk').value = data.tahun_masuk;
                document.getElementById('edit-sparepart-tanggal-masuk').value = data.tanggal_masuk;
                document.getElementById('edit-sparepart-merk').value = data.merk;
                document.getElementById('edit-sparepart-tipe').value = data.tipe;
                document.getElementById('edit-sparepart-serial-number').value = data.serial_number;
                document.getElementById('edit-sparepart-sumber-pengadaan').value = data.sumber_pengadaan;
                document.getElementById('edit-sparepart-status').value = data.status;
                document.getElementById('edit-sparepart-tanggal-keluar').value = data.tanggal_keluar;
                document.getElementById('edit-sparepart-lokasi-pemasangan').value = data.lokasi_pemasangan;
                document.getElementById('edit-sparepart-lokasi-pengiriman').value = data.lokasi_pengiriman;
                document.getElementById('edit-sparepart-keterangan').value = data.keterangan;
                if(data.berkas != null){
                document.getElementById('edit-sparepart-file-preview').innerHTML = `
                <a href="/storage/${data.berkas}" target="_blank">Lihat Dokumen Lama</a>
                `;
                }
               
                // document.getElementById('edit-hardware-status').value = data.status;
                const statusSparepartEdit = document.getElementById('edit-sparepart-status').value;
                const inputTanggalKeluar = document.getElementById('edit-sparepart-tanggal-keluar');
                const divTanggalKeluar = document.getElementById('divTanggalKeluarEdit');
                const inputLokasiPengiriman = document.getElementById('edit-sparepart-lokasi-pengiriman');
                const divLokasiPengiriman = document.getElementById('divLokasiPengirimanEdit');
                const inputLokasiPemasangan = document.getElementById('edit-sparepart-lokasi-pemasangan');
                const divLokasiPemasangan = document.getElementById('divLokasiPemasanganEdit');
                if (statusSparepartEdit === 'ready') {
                    divTanggalKeluar.style.display = 'none';
                    inputTanggalKeluar.required = false;
                    divLokasiPengiriman.style.display = 'none';
                    inputLokasiPengiriman.required = false;
                    divLokasiPemasangan.style.display = 'none';
                    inputLokasiPemasangan.required = false;
                }else if(statusSparepartEdit === 'terpasang'){
                    divTanggalKeluar.style.display = 'block';
                    inputTanggalKeluar.required = true;
                    divLokasiPemasangan.style.display = 'block';
                    inputLokasiPemasangan.required = true;
                    divLokasiPengiriman.style.display = 'none';
                    inputLokasiPengiriman.required = false;
                    // pelepasanInput.value = ''; // optional: reset value
                }
                else if(statusSparepartEdit === 'terkirim'){
                    divTanggalKeluar.style.display = 'block';
                    inputTanggalKeluar.required = true;
                    divLokasiPemasangan.style.display = 'none';
                    inputLokasiPemasangan.required = false;
                    divLokasiPengiriman.style.display = 'block';
                    inputLokasiPengiriman.required = true;
                    // pelepasanInput.value = ''; // optional: reset value
                }
                
                
                const modal = new bootstrap.Modal(document.getElementById('modalEditSparepart'));
                // if (document.getElementById('edit-hardware-status').value === 'dilepas') {
                //     document.getElementById('inputTanggalPelepasanEdit').style.display = 'block';
                // } else {
                //     document.getElementById('inputTanggalPelepasanEdit').style.display = 'none';
                // }
                modal.show();

            });
    });
});
}
tambahEventListenerBtnEdit();

document.getElementById('edit-sparepart-status').addEventListener('change', function(e) {
// const pelepasanInput = document.getElementById('inputTanggalPelepasan');
const inputTanggalKeluar = document.getElementById('edit-sparepart-tanggal-keluar');
const divTanggalKeluar = document.getElementById('divTanggalKeluarEdit');
const inputLokasiPengiriman = document.getElementById('edit-sparepart-lokasi-pengiriman');
const divLokasiPengiriman = document.getElementById('divLokasiPengirimanEdit');
const inputLokasiPemasangan = document.getElementById('edit-sparepart-lokasi-pemasangan');
const divLokasiPemasangan = document.getElementById('divLokasiPemasanganEdit');
if (e.target.value === 'ready') {
    divTanggalKeluar.style.display = 'none';
    inputTanggalKeluar.required = false;
    divLokasiPengiriman.style.display = 'none';
    inputLokasiPengiriman.required = false;
    divLokasiPemasangan.style.display = 'none';
    inputLokasiPemasangan.required = false;
}else if(e.target.value === 'terpasang'){
    divTanggalKeluar.style.display = 'block';
    inputTanggalKeluar.required = true;
    divLokasiPemasangan.style.display = 'block';
    inputLokasiPemasangan.required = true;
    divLokasiPengiriman.style.display = 'none';
    inputLokasiPengiriman.required = false;
    // pelepasanInput.value = ''; // optional: reset value
}
else if(e.target.value === 'terkirim'){
    divTanggalKeluar.style.display = 'block';
    inputTanggalKeluar.required = true;
    divLokasiPemasangan.style.display = 'none';
    inputLokasiPemasangan.required = false;
    divLokasiPengiriman.style.display = 'block';
    inputLokasiPengiriman.required = true;
    // pelepasanInput.value = ''; // optional: reset value
}
});

document.getElementById('formEditSparepart').addEventListener('submit', function (e) {
    e.preventDefault();
    const id = document.getElementById('edit-sparepart-id').value;
    const formData = new FormData(this);

    fetch(`/sparepart/${id}`, {
        method: 'POST',
        headers: {
            'X-CSRF-TOKEN': formData.get('_token'),
            'X-HTTP-Method-Override': 'PUT'
        },
        body: formData
    })
    .then(res => res.json())
    .then(data => {
        if (data.success) {
            location.reload(); // atau update baris tabel secara dinamis
        }
    });
});
</script>
{{-- END Edit Sparepart --}}

{{-- DATA TABLE --}}
<script>
$(document).ready(function() {
    $('#sparepart-table').DataTable({
        info: false,
        lengthChange: false,
        pageLength: 15
    });
});
</script>
{{-- END DATA TABLE --}}
@endsection